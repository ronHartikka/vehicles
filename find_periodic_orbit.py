...
def integrate_to_section(state0, rhs, event, t_max=10000.0,
    ...
    """Integrate from state0 until the next Poincare section crossing.

    Phase 1: burn-in integration (t_burnin time units) without events
             to move off the section and avoid a spurious t=0 trigger.
    Phase 2: integrate with event detection until terminal crossing.
...
def return_map(free_vars, rhs, event, theta0=0.0, t_max=10000.0,
               rtol=1e-10, atol=1e-12, t_burnin=0.5):
    """Poincare return map: (x0, y0) -> (x1, y1).

    Integrates from [x0, y0, theta0] until the next section crossing.
    Returns (np.array([x1, y1]), period).
    """
    x0, y0 = free_vars
    state0 = [x0, y0, theta0]
    state_cross, period, _ = integrate_to_section(
        state0, rhs, event, t_max, rtol, atol, t_burnin)
    return np.array([state_cross[0], state_cross[1]]), period


def shooting_residual(free_vars, rhs, event, theta0=0.0, t_max=10000.0,
                      rtol=1e-10, atol=1e-12, t_burnin=0.5):
    """Residual for shooting: F(q) = return_map(q) - q."""
    q = np.array(free_vars)
    try:
        q_return, _ = return_map(free_vars, rhs, event, theta0,
                                 t_max, rtol, atol, t_burnin)
        return q_return - q
...
def find_periodic_orbit(guess, rhs, event, theta0=0.0, t_max=10000.0,
                        rtol=1e-10, atol=1e-12, t_burnin=0.5,
                        verbose=False):
    ...
    """
    if verbose:
        print(f"  Initial guess: ({guess[0]:.4f}, {guess[1]:.4f})")
        r0 = shooting_residual(guess, rhs, event, theta0,
                               t_max, rtol, atol, t_burnin)
    ...
    result, info, ier, msg = fsolve(
        ...
        args=(rhs, event, theta0, t_max, rtol, atol, t_burnin),
        full_output=True)

    # Compute final residual and period
    final_residual = shooting_residual(
        result, rhs, event, theta0, t_max, rtol, atol, t_burnin)
    try:
        _, period = return_map(result, rhs, event, theta0,
                               t_max, rtol, atol, t_burnin)
    except RuntimeError:
        period = float('nan')
    ...
    if verbose:
        ...
        print(f"  Fixed point: ({result[0]:.6f}, {result[1]:.6f})")
...
def compute_floquet_multipliers(fixed_point, rhs, event, theta0=0.0,
                                ...
                                t_burnin=0.5, eps=1e-6, verbose=False):
    ...
    f0, _ = return_map(q0, rhs, event, theta0, t_max, rtol, atol, t_burnin)

    J = np.zeros((2, 2))
    for j in range(2):
        dq = np.zeros(2)
        dq[j] = eps
        f_plus, _ = return_map(q0 + dq, rhs, event, theta0,
                               t_max, rtol, atol, t_burnin)
        f_minus, _ = return_map(q0 - dq, rhs, event, theta0,
                                t_max, rtol, atol, t_burnin)
...
def continuation_sweep(fixed_point, config, vehicle, param_name,
                       ...
                       section_type="theta", theta0=0.0, t_max=10000.0,
                       rtol=1e-10, atol=1e-12, t_burnin=0.5,
                       verbose=False):
    ...
    for i, pval in enumerate(param_values):
        ...
        event = make_poincare_event(section_type, theta0)

        try:
            fp, period, info = find_periodic_orbit(
                current_guess, rhs, event, theta0, t_max, rtol, atol,
                t_burnin, verbose=verbose)

            mults = compute_floquet_multipliers(
                fp, rhs, event, theta0, t_max, rtol, atol, t_burnin,
                verbose=verbose)
...
def plot_results(fixed_point, period, rhs, event, theta0, config, vehicle,
    ...
    # Integrate the full orbit for plotting (burn-in + main)
...
def main():
    ...
    print(f"\nPoincare section: {args.section} = {section_value:.4f}, "
          f"direction = +1")
    print(f"Initial guess: ({x0:.4f}, {y0:.4f}), theta0 = {theta0:.4f}")
    ...
    try:
        fixed_point, period, info = find_periodic_orbit(
            guess, rhs, event, theta0, args.t_max,
            args.rtol, args.atol, t_burnin=args.t_burnin, verbose=True)
    ...
    if converged or info['residual_norm'] < 1.0:
        ...
        try:
            multipliers = compute_floquet_multipliers(
                fixed_point, rhs, event, theta0, args.t_max,
                args.rtol, args.atol, t_burnin=args.t_burnin, verbose=True)
        except Exception as e:
    ...
    if (args.continuation_param
        ...
        continuation_results = continuation_sweep(
            ...
            args.section, theta0, args.t_max, args.rtol, args.atol,
            t_burnin=args.t_burnin, verbose=args.verbose)

        # Print summary table
        print(f"\n  {'Param':>12s} {'x':>10s} {'y':>10s} "
